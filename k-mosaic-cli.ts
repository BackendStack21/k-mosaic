...
kem
  ...
  .action(
    async (options: {
      ...
      const keyFileData = await fs.readFile(options.publicKey, 'utf-8')
      const keyFile = JSON.parse(keyFileData)
      const publicKeyBytes = Buffer.from(keyFile.public_key, 'base64')
...
kem
  ...
  .action(
    async (options: {
      ...
      const ciphertextData = await fs.readFile(options.ciphertext, 'utf-8')
      const ciphertextFile = JSON.parse(ciphertextData)
      const encryptedBuffer = Buffer.from(ciphertextFile.ciphertext, 'base64')
      // Create a properly aligned copy for Int32Array views
      const encrypted = new Uint8Array(encryptedBuffer.length)
      encrypted.set(encryptedBuffer)

      const decrypted = await decrypt(encrypted, secretKey, publicKey)

      await writeOutput(decrypted, options.output)
    },
  )

kem
  .command('encapsulate')
  .description('Create a shared secret and ciphertext')
  .requiredOption('-p, --public-key <path>', 'Path to public key file')
  .option('-o, --output <path>', 'Output file path')
  .action(async (options: { publicKey: string; output?: string }) => {
    ...
    const output = {
      ciphertext: Buffer.from(serializeCiphertext(ciphertext)).toString(
        'base64',
      ),
      shared_secret: Buffer.from(sharedSecret).toString('base64'),
    }
...
kem
  .command('decapsulate')
  .description('Recover a shared secret')
  .requiredOption('-s, --secret-key <path>', 'Path to secret key file')
  .requiredOption('-p, --public-key <path>', 'Path to public key file')
  .requiredOption('-c, --ciphertext <path>', 'Path to ciphertext file')
  .option('-o, --output <path>', 'Output file path')
  .action(
    async (options: {
      ...
      const ciphertextData = await fs.readFile(options.ciphertext, 'utf-8')
      const ciphertextFile = JSON.parse(ciphertextData)
      const ciphertextBuffer = Buffer.from(ciphertextFile.ciphertext, 'base64')
      // Create a properly aligned copy for Int32Array views
